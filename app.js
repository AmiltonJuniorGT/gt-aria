  ...
  view.innerHTML = `
    ...
    ">
      <div style="display:flex;gap:12px;align-items:flex-end;justify-content:space-between;flex-wrap:wrap;">
        <div>
          <div style="
            ...
            background:linear-gradient(90deg,#dff6e8 0%,#b8e3c1 100%);
            color:#1b5e20;
            font-weight:900;
            font-size:12px;
            padding:6px 12px;
            ...
            margin-bottom:8px;
            letter-spacing:.03em;
          ">LURUS • ACELERADOR</div>

          <h2 style="margin:0 0 4px 0;color:#123b2d;">Qualificação de Leads (HUB)</h2>
          <div style="opacity:.9;font-size:13px;color:#234236;">
          ...
          <div style="opacity:.8;font-size:12px;margin-top:6px;color:#335548;">
      ...
      <div style="
        ...
        margin-bottom:14px;
      ">
        ${renderTopStat("Base", formatInt(totalAll))}
        ${renderTopStat("FinalizadoM", formatInt(totalMat))}
        ${renderTopStat("Marca", state.marcaSelecionada === "AMBOS" ? "Ambos" : normalizeMarcaLabel(state.marcaSelecionada))}
        ${renderTopStat("Vendedor", state.vendedorSelecionado === "TODOS" ? "Todos" : state.vendedorSelecionado)}
      </div>
      ...
      <div class="row" style="align-items:flex-end;">
        ...
        <div class="ctrl" style="min-width:320px;flex:1;">
          <label>Buscar (CPF/Nome/Fone)</label>
          <input id="inpBusca" class="input" placeholder="Digite CPF, nome ou telefone..." value="${escapeAttr(state.busca)}" />
        </div>
      </div>
    ...
    <div class="card" style="
      ...
      background:#fcfefd;
    ">
      <h3 style="margin:0 0 8px 0;color:#123b2d;">Classificação (dentro do Vendedor) — arraste para reordenar</h3>
    ...
    ">
      <div class="rowBetween">
        <div>
          <h3 style="margin:0;color:#123b2d;">Resultado (agrupado por Vendedor)</h3>
          <div class="muted small" id="resultadoInfo">Clique em <b>Gerar Lista</b>.</div>
        </div>
      </div>

      <div class="tableWrap" style="margin-top:12px;border-radius:18px;overflow:auto;border:1px solid #dcefe1;">
        <table id="tbl">
          <thead>
            <tr>
              <th>#</th>
...
function renderTopStat(label, value) {
  return `
    <div style="
      ...
      background:linear-gradient(180deg,#f6fcf8 0%,#ebf7ee 100%);
      border-radius:16px;
      padding:12px 14px;
    ">
      <div style="font-size:11px;color:#3f6b55;font-weight:800;letter-spacing:.04em;text-transform:uppercase;">${escapeHtml(label)}</div>
      <div style="font-size:20px;color:#184d36;font-weight:900;margin-top:4px;">${escapeHtml(String(value))}</div>
    </div>
  `;
}
...
function renderAICard(title, subtitle, items) {
  const listHtml = (items || [])
    .map((item, index) => {
      return `
        <li style="display:grid;grid-template-columns:32px 1fr;gap:10px;align-items:flex-start;">
          <div style="
            width:32px;
            height:32px;
            border-radius:999px;
            background:linear-gradient(180deg,#34a853 0%,#2f8c47 100%);
            color:#fff;
            font-weight:900;
            font-size:13px;
            display:flex;
            ...
            box-shadow:0 4px 10px rgba(52,168,83,.22);
          ">
            ${index + 1}
          </div>
          <div style="color:#14532d;font-size:14px;line-height:1.5;font-weight:700;">${escapeHtml(item)}</div>
        </li>
      `;
    })
    .join("");
  ...
  return `
    ...
    ">
      <div style="
        display:inline-block;
        background:linear-gradient(90deg,#cdeed7 0%,#a7ddb7 100%);
        color:#1c5a31;
        font-weight:900;
        font-size:12px;
        padding:6px 10px;
      ...
      <div style="font-size:13px;color:#227544;font-weight:700;margin-bottom:14px;">
        ${escapeHtml(subtitle)}
      </div>
...
function renderDrag() {
  ...
  state.prioridade.forEach((p) => {
    ...
    div.draggable = true;
    div.textContent = p;
    div.style.border = "1px solid #bfe3c8";
    div.style.borderRadius = "14px";
    div.style.padding = "10px 14px";
    div.style.marginBottom = "8px";
    div.style.background = "linear-gradient(180deg,#f5fcf7 0%,#e7f6ec 100%)";
    div.style.color = "#1c5a31";
...
function renderTabelaGrouped(vendedores, groups) {
  ...
  for (const vend of vendedores) {
    ...
    html.push(`
      <tr>
        <td colspan="${ENABLE_SCORE_IA ? 14 : 13}" style="background:linear-gradient(90deg,#eaf8ee 0%,#d7f0de 100%);border-bottom:1px solid #cfe8d5;">
          <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;padding:4px 2px;">
            <div style="font-weight:1000;color:#184d36;">${escapeHtml(vend)}</div>
            <div style="opacity:.85;font-size:12px;color:#24523d;">${list.length} leads</div>
          </div>
        </td>
      </tr>
    `);
...
function renderWhatsButton(link) {
  return `
    <a
      href="${link}"
      target="_blank"
